...
They are independent sample projections on the retained conductance manifold, not
uniquely identifiable analytical concentrations or molecular radii.
"""
...
INTERPRETATION_WARNING = (
    "Independent sample coordinates are conductance-equivalent projections, "
    "not uniquely identifiable physical concentrations or molecular radii."
)
...
def _prepare_input(path: Path) -> pd.DataFrame:
    ...
    bad_intercepts = df.loc[df["intercept"] <= 0, "sample_id"].tolist()
    if bad_intercepts:
        raise ValueError(f"Samples with non-positive intercepts cannot be fitted: {bad_intercepts}")
    negative_slopes = df.loc[df["slope"] < -SIGN_TOL, "sample_id"].tolist()
    if negative_slopes:
        raise ValueError(
            "Samples with negative concentration-penalty slopes are outside "
            f"the retained CPTDS model: {negative_slopes}"
        )
    df.loc[(df["slope"] < 0) & (df["slope"].abs() <= SIGN_TOL), "slope"] = 0.0
        ...
        [
            {
                ...
                "blocker_size_max": float(max(config.blocker_size_values)),
                ...
                "fit_mode": "independent_three_coordinate_projection",
...
def main() -> None:
    ...
    parser.add_argument("--dimension", type=int, choices=[3], default=3, help="Monte Carlo box dimension. The public retained model is 3D.")
